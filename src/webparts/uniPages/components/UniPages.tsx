...
//@ts-ignore
private async getMyAdmissionSafeDateForUniversity(title: string): Promise<string | null> {
  try {
  const uni: any[] = await this._sp.web.lists
  .getByTitle("University")
  .items.filter(`Title eq '${title}'`)
...
const MyAdmissionSafeDate = await this.getMyAdmissionSafeDateForUniversity(title);
